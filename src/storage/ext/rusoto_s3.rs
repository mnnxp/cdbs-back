//! AWS Post Policy
//!
//! From [comment](https://github.com/rusoto/rusoto/pull/1807#issuecomment-851314417)
//! Follows [POST Policy
        ...
        if v.0 == "content-length-range" {
            seq.serialize_element(&v.1.parse::<u64>().map_err(|_| {
                serde::ser::Error::custom(
                    "expected u64 value, the minimum content length",
                )
            })?)?;
            seq.serialize_element(&v.2.parse::<u64>().map_err(|_| {
                serde::ser::Error::custom(
                    "expected u64 value, the maximum content length",
                )
            })?)?;
        } else {
...
impl<'a> PostPolicy<'a> {
    ...
    pub fn set_security_token(mut self, security_token: &'a str) -> Self {
        ...
        self =
            self.append_policy("eq", "$x-amz-security-token", security_token);
        self
    }
    ...
    /// the field to the form data produced by
    /// [build_form_data](#method.build_form_data)
    ...
    pub fn build_form_data(
        ...
        let policy_as_json = serde_json::to_string(&policy_to_serialize)
            .map_err(|e| format!("{:?}", e))?;
        ...
        let upload_url = if cfg!(feature = "aws-local") {
            // use path format for the url.
            // since we are running on localhost anyway!
            format!(
                "{}://{}/{}",
...
fn hmac(secret: &[u8], message: &[u8]) -> Hmac<Sha256> {
    let mut hmac =
        Hmac::<Sha256>::new_from_slice(secret).expect("failed to create hmac");
    hmac.update(message);
    hmac
}

/// Takes a message and signs it using AWS secret, time, region keys and service
/// keys.
fn sign_string(
    string_to_sign: &str,
    secret: &str,
...
mod tests {
    ...
    fn build_successfully() {
        ...
        assert!(res.is_ok());
        let (upload_url, form_data) = res.unwrap();
        if cfg!(feature = "aws-local") {
            assert_eq!(
                upload_url,
                "https://s3.eu-central-1.amazonaws.com/the-bucket"
            );
        } else {
            assert_eq!(
                upload_url,
        ...
        let policy: serde_json::Value =
            serde_json::from_slice(&policy_as_vec_u8).unwrap();
