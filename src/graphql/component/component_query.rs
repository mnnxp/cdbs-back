...
impl ComponentQuery {
    ...
    async fn components(
        ...
        let options = ExtraOptions::from_ctx(ctx, !arguments.favorite)?;
        let s = sort
            .map(|s| Sort::parsing(TableName::ComponentRef, &s.by_field, s.as_desc))
            .unwrap_or(Sort::set_by_table(TableName::ComponentRef));
        let p = paginate
    ...
    async fn component_suppliers(
        ...
        // authorization check, if token verification fails, try to get the default user UUID
        let logged_user_uuid = ExtraOptions::from_ctx(ctx, true).map(|eo| eo.logged_user_uuid)?;
        let conn: &mut PooledConnection = &mut get_conn(ctx)?;
        let p = paginate
            .map(|p| Paginate::parsing_by_page(p.current_page, p.per_page))
            .unwrap_or_default();
    ...
    async fn component_types(
        ...
        get_component_types(&filter, get_set_language(ctx), conn)
    }
    ...
    async fn component_actual_statuses(
        ...
        get_actual_statuses(&filter, get_set_language(ctx), conn)
    }
}
