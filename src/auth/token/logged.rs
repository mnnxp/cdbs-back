...
use crate::auth::token::{find_user_by_token, token_from_cxt};
...
pub(crate) fn check_authorized(cxt: &Context<'_>) -> ServiceResult<bool> {
    get_logged_user_uuid(cxt).map(|user_uuid| !user_uuid.is_nil())
}

/// Retrieves the UUID of the authenticated user from the GraphQL context.
/// Send query to database token validation.
pub(crate) fn get_logged_user_uuid(cxt: &Context<'_>) -> ServiceResult<Uuid> {
    // Check auth context from middleware first
    if let Some(auth_ctx) = cxt.data_opt::<AuthStatus>().as_ref() {
        match auth_ctx {
