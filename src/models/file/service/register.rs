...
pub(crate) fn register(file_data: FileData, pool: web::Data<Pool>) -> ServiceResult<SlimFile> {
    let conn = &db_connection(&pool)?;
    create_file(file_data, conn)
}

pub(crate) fn create_file(file_data: FileData, conn: &PgConnection) -> ServiceResult<SlimFile> {
