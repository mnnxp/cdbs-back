...
pub(crate) fn create_discussion_comment(
    ...
    let valid_discussion_uuid = match discussion_to.get_associated_discussion_uuid(&data.discussion_uuid, conn) {
        Ok(d_uuid) => d_uuid,
        Err(err) => {
            debug!("Failed valid discussion data: {:?}, discussion_uuid of data: {:?}", err, data.discussion_uuid);
            if data.discussion_uuid.is_some() {
                return Err(err);
            }
            let new_discussion_uuid = create_discussion_without_check(String::from("First"), true, conn)?;
    ...
    if let Some(parent_comment_uuid) = data.parent_comment_uuid {
        ...
        if valid_discussion_uuid == dc_data.discussion_uuid {
            ...
            debug!("Parent's comment {:?} not found in the discussion {:?}", parent_comment_uuid, valid_discussion_uuid);
...
fn set_self_parent_comment(
    comment_uuid: &Uuid,
    conn: &mut PgConnection
) -> ServiceResult<usize> {
    diesel::update(discussion_comment_list::discussion_comment_list
