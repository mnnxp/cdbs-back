...
pub(crate) fn get_standard(
    ...
    let target_standards_uuids: Vec<Uuid> = match (company_uuid, favorite) {
        ...
        (None, true) => get_standards_followed_by_user(
            filter_standards_uuids,
        ...
        (None, false) => filter_standards_uuids.to_vec(),
    ...
    ShowStandardShort::get_standards(&target_standards_uuids, options, paginate, conn).map_err(
        |err| {
            debug!(
                "Error loading list standards and collect short data: {:?}",
                err
            );
...
fn get_standards_by_user(
    ...
    query = match filter_standards_uuids.is_empty() {
        true => query.filter(standard_ref::company_uuid.eq(company_uuid)),
        false => query.filter(
...
pub(crate) fn find_by_uuid(
    ...
    let result: StandardAndRelatedData =
        StandardAndRelatedData::collect_related_data(target_standard_uuid, options, paginate, conn)
