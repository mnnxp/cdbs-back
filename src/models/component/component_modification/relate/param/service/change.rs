    ...
    for param_d in &data.params {
        if param_d.param_id > 0 { // <-- additionally we check the correctness of the id
            ...
            let get_param = param_to_modification::param_to_modification
                .filter(param_to_modification::modification_uuid.eq(&data.modification_uuid)
...
fn update_modification_params_values(
    ...
    for param_d in data {
        let insert_params = diesel::update(param_to_modification::param_to_modification
            .filter(param_to_modification::modification_uuid.eq(target_modification_uuid)
            .and(param_to_modification::param_id.eq(&param_d.param_id))))
