...
use crate::models::supplier_service::access::company::model::{
    ...
    CompanyAccessServiceAndRelatedData,
    IptCompanyAccessServiceData,
    InsertableCompanyAccessService,
    DelCompanyAccessServiceData,
};
...
pub(crate) fn get_companies_list_access_service(
    ...
    match list_companies_with_access {
        ...
        Err(err) => {
            debug!("Failed get companies list have access to service: {:?}", err);
            Err(get_err_msg(ErrorMessage::FailedGetCompaniesWithAccessService))
        },
    }
}
...
pub(crate) fn set_company_access_service(
    ...
    let set_access = diesel::update(company_access_to_service
