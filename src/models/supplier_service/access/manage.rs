...
use crate::models::supplier_service::access::model::{
    ChangeOwnerService, ChangeTypeAccessService,
};
...
pub(crate) fn change_service_owner_user(
    ...
    let change_owner = diesel::update(service_ref::service_ref
        .filter(service_ref::uuid.eq(&data.service_uuid)
...
pub(crate) fn change_service_type_access(
    ...
    let change_access = diesel::update(service_ref::service_ref
        .filter(service_ref::uuid.eq(&data.service_uuid)
        .and(service_ref::user_uuid.eq(&options.logged_user_uuid))))
