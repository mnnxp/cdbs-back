...
pub(crate) fn del_service_params(
    ...
    let old_data: Vec<(i32, String)> = param_to_service::param_to_service
        .filter((param_to_service::service_uuid.eq(&data.service_uuid))
...
fn delete_service_params_values(
    ...
    diesel::delete(param_to_service::param_to_service
        .filter(param_to_service::service_uuid.eq(target_service_uuid)
