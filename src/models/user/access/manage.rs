...
use crate::auth::token::{
    decode, delete_all_tokens, delete_user_token, show_tokens, token_from_ctx, update,
};
...
pub(crate) fn check_token_valid(ctx: &Context<'_>, conn: &mut PgConnection) -> ServiceResult<bool> {
    let token_str = token_from_ctx(ctx)?;
...
pub(crate) fn get_token_days_until_expiry(ctx: &Context<'_>) -> ServiceResult<i64> {
    let token_str = token_from_ctx(ctx)?;
...
pub(crate) fn get_user_token(ctx: &Context<'_>) -> ServiceResult<Token> {
    update(ctx, false)
}

/// Generates a new token and deactivates the current token.
/// Returns the user's new authorization token.
pub(crate) fn update_user_token(ctx: &Context<'_>) -> ServiceResult<Token> {
    update(ctx, true)
}

/// Returns token provider, user UUID, username, program ID, issued at, and expiration date.
pub(crate) fn decode_user_token(ctx: &Context<'_>) -> ServiceResult<Claims> {
    decode(&token_from_ctx(ctx)?)
}
